            ...
            >
              <template v-slot:body>
                ...
                >
                  ...
                  <td class="px-4 py-2 border border-gray-200 ">{{ item.file_name }}</td>
                  <td class="px-4 py-2 border border-gray-200">{{ item.folder_name }}</td>
                  <td class="px-4 py-2 border border-gray-200 text-end">{{ item.file_size }}</td>
                  <td class="px-4 py-2 border border-gray-200 text-center">{{ item.created_at }}</td>
      ...
      <template v-slot:formdata>
        ...
        <div class="mb-4">
          <label
            ...
            class="block text-sm font-medium text-cyan-700"
          >Avatar</label>
          <UFileUpload v-model="record.avatar" />
        </div>
...
export default {
  ...
  setup() {
    ...
    const endpoint = "api/v1/utilities/media-files";

    const winheight = window.innerHeight - 320;
    const headers = [
      { title: "#", key: "ids", align: "center", width: "45px" },
      { title: "FILE", key: "name", align: "start" },
      { title: "FOLDER", key: "email", align: "start" },
      { title: "UKURAN (Kb)", key: "authent", align: "center" },
      { title: "TANGGAL", key: "created_at", align: "center" },
    ...
    onMounted(() => {
      store.setPage({
        ...
        subtitle: "Berikut Daftar File Yang Tersedia Aplikasi ",
        breadcrumbs: [
          {
            name: "",
            title: "Utility",
