<template>
    <div class="flex flex-col mt-4 w-full min-h-full animate-fadeIn ">
            ...
            <div class="lg:hidden">
              <u-data-mobile>
                <template v-slot:data>
                  <li
                    v-for="(item,index) in records"
                    class="bg-white text-sm flex items-center justify-between p-4 rounded shadow hover:bg-cyan-100 sm:flex-col"
                  >
      ...
      <UFormDialog
        title="Formulir Manajemen Bidang"
        @onSave="postRecord"
        @onUpdate="postUpdate"
      >
  ...
  export default {
    ...
    setup() {
      ...
      const endpoint = "api/v1/master-data/division-master";
