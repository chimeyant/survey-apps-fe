    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="bg-white rounded-lg shadow-xl p-8">
        ...
        >
          ...
          <div
            v-for="document in formData.attachments"
            :key="document.id"
          >
...
const fetchSurveyTopicCategories = async () => {
  try {
    const response = await store.getCombo(
      "/api/v1/combo/survey-topic-categories/" + formData.survey_topic_id,
      true
    );
...
const fetchRegencies = async (provinceCode) => {
  try {
    const response = await fetch(
      `/api/v1/regencies?province_code=${provinceCode}`
    );
    const result = await response.json();
...
const fetchVillages = async (districtCode) => {
  try {
    const response = await store.getCombo(
      "/api/v1/combo/villages/" + districtCode,
      true
    );
...
const fetchOtherDocuments = async () => {
  try {
    const response = await store.fetchRecords(
      "/api/v1/survey/survey-topic-documents-all/" + formData.survey_topic_id,
      {},
      true
    );
    console.log(response);
...
const addOtherDocument = (payload, document) => {
  ...
  if (payload.value != null) {
    ...
    const idx = formData.attachments.findIndex(
